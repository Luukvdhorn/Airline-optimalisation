...
def main():
    ...
    # # xrp[r,p] = passagiers die van p naar r gaan (after recapture)
    # xrp = model.addVars(P, P, lb=0, vtype=GRB.INTEGER, name="xrp")

    #     # LINKING CONSTRAINTS
    # for p in P:
    #     # x[p,p] = passagiers die op eigen itinerary blijven
    #     model.addConstr(x[p, p] == D[p] - quicksum(t[r, p] for r in P if r != p), name=f"link_x_{p}")

    # for r in P:
    #     for p in P:
    #         if r != p:
    #             model.addConstr(xrp[r, p] == b_pr.get((r, p), 0) * t[r, p], name=f"link_xrp_{r}_{p}")
    #         else:
    #             model.addConstr(xrp[r, p] == 0)  # xrp[p,p] = 0

    # OBJECTIVE FUNCTION
    model.setObjective(quicksum((fare[p] - b_pr.get((p, r), 0) * fare[r]) * t[r, p] for p in P for r in P),
        GRB.MINIMIZE)
    

    # CAPACITY CONSTRAINTS
    for l in L:
        model.addConstr(quicksum(delta_lp.get((l, p), 0) * t[r, p] for p in P for r in P)
            - quicksum(delta_lp.get((l, r), 0) * b_pr.get((p, r), 0) * t[p, r] for p in P for r in P)
            >= Q[l] - CAP[l], name=f"cap_{l}")

    # DEMAND CONSTRAINTS
    for p in P:
        model.addConstr(quicksum(t[r, p] for r in P) <= D[p], name=f"demand_{p}")


    for p in P:
        for r in P:
            model.addConstr(t[r, p] >= 0, name=f"nonneg_{r}_{p}") # met lb = 0 ook al gevangen
    # RESULTS
    model.optimize()
    ...
    if model.status in [GRB.OPTIMAL, GRB.TIME_LIMIT]:
        ...
        # print("x[p,p] (passagiers op eigen itinerary):")
        # for p in P:
        #     if x[p,p].x > 1e-6:
        #         print(f"x[{p},{p}] = {x[p,p].x}")
        # print("\nxrp[r,p] (passagiers die worden herverdeeld):")
        # for r in P:
        #     for p in P:
        #         if xrp[r, p].x > 1e-6:
        #             print(f"xrp[{r},{p}] = {xrp[r,p].x}")
        # print("\nt[r,p] (aantal herverdeelde passagiers):")
        for r in P:
            for p in P:
                if t[r, p].x > 1e-6:
                    print(f"t[{r},{p}] = {t[r,p].x}")   
    else:
