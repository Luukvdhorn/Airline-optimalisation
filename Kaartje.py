...
for i in N:                                 #YIELD
    for j in N:
        if d[i, j] > 0:
            yieldm[i, j] = (5.9 * d[i, j]**(-0.76) + 0.043)
        else:
            yieldm[i, j] = 0   
...
def main():
    ...
    Objective_1B = quicksum((yieldm[i, j] * d[i, j] * (x[i, j] + w[i, j])) for i in N for j in N) - quicksum(Ck_ij[i, j, k] * z[i, j, k] for k in K for i in N for j in N) - quicksum(cl[k] * AC[k] for k in K)
    ...
    if model.status == GRB.OPTIMAL or model.status == GRB.TIME_LIMIT:
        print(f"Totaal winst: €{model.ObjVal:.2f}")
        for k in K:
            print(f"{df_aircraft.index[k]}: {AC[k].X:.0f}")
        
        totaal_passagiers = sum(x[i,j].X for i in N for j in N)
        print(f"Totaal passagiers: {totaal_passagiers:.0f}")
        
        transfer_passagiers = sum(w[i,j].X for i in N for j in N)
        print(f"Passagiers transfer: {transfer_passagiers}")

        # --- routes per vliegtuigtype ---
        routes = []
        colors = ['blue','black','green','purple','red']  # kleur per vliegtuigtype
        for i in N:
            for j in N:
                if i != j:
                    for k in K:
                        flights = z[i,j,k].X
                        if flights > 0.5:
                            routes.append({'i':i,'j':j,'type':k,'flights':flights})
        ...
        airports_df = pd.DataFrame({'icao':airports,'lon':longitudes,'lat':latitudes})
        geometry = [Point(xy) for xy in zip(airports_df['lon'],airports_df['lat'])]
        gdf_airports = gpd.GeoDataFrame(airports_df, geometry=geometry, crs="EPSG:4326")

        lines = []
        line_colors = []
        for r in routes:
            line = LineString([(longitudes[r['i']], latitudes[r['i']]), (longitudes[r['j']], latitudes[r['j']])])
            lines.append({'geometry':line, 'flights':r['flights'], 'color':colors[r['type'] % len(colors)]})

        gdf_routes = gpd.GeoDataFrame(lines, crs="EPSG:4326")

        # Convert CRS voor basemap
        gdf_airports = gdf_airports.to_crs(epsg=3857)
        gdf_routes = gdf_routes.to_crs(epsg=3857)

        fig, ax = plt.subplots(figsize=(12,12))
        for idx, row in gdf_routes.iterrows():
            gpd.GeoSeries([row.geometry]).plot(ax=ax, linewidth=row['flights']*0.5, color=row['color'], alpha=0.7)

        gdf_airports.plot(ax=ax, color=['red' if i==hub_index else 'black' for i in range(len(gdf_airports))], markersize=50)
        ctx.add_basemap(ax, source=ctx.providers.OpenStreetMap.Mapnik)

        for x,y,label in zip(gdf_airports.geometry.x, gdf_airports.geometry.y, gdf_airports['icao']):
            ax.text(x+20000, y+20000, label, fontsize=8)

        plt.title("Network map, flights")
        plt.axis('off')
        plt.show()
